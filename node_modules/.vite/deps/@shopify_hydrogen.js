...
import {
  ...
  useNavigate,
  useNavigation
} from "./chunk-FKQMKPBX.js";
import {
  require_react
} from "./chunk-KS2TIU2K.js";
import {
  __privateAdd,
  __privateGet,
  __privateSet,
...
// node_modules/@shopify/hydrogen-react/dist/browser-dev/node_modules/use-isomorphic-layout-effect/dist/use-isomorphic-layout-effect.browser.esm.mjs
...
function cartDiscountCodesUpdateDefault(options) {
  return async (discountCodes, optionalParams) => {
    const uniqueCodes = discountCodes.filter((value, index2, array) => {
...
function cartGiftCardCodesUpdateDefault(options) {
  return async (giftCardCodes, optionalParams) => {
    const uniqueCodes = giftCardCodes.filter((value, index2, array) => {
      return array.indexOf(value) === index2;
    });
  ...
  for (const { formData } of fetchers) {
    ...
    if (cartFormData.action === CartForm.ACTIONS.LinesAdd) {
      ...
      for (const lineId of cartFormData.inputs.lineIds) {
        const index2 = cartLines.findIndex((line) => line.id === lineId);
        if (index2 !== -1) {
          if (isOptimisticLineId(cartLines[index2].id)) {
            console.error(
              "[h2:error:useOptimisticCart] Tried to remove an optimistic line that has not been added to the cart yet"
            );
            continue;
          }
          cartLines.splice(index2, 1);
          isOptimistic = true;
        } else {
          console.warn(
      ...
      for (const line of cartFormData.inputs.lines) {
        const index2 = cartLines.findIndex(
        ...
        if (index2 > -1) {
          ...
          cartLines[index2].quantity = line.quantity;
          if (cartLines[index2].quantity === 0) {
            cartLines.splice(index2, 1);
          }
          isOptimistic = true;
        } else {
...
var DEFAULT_CUSTOMER_API_VERSION = "2025-04";
...
async function hydrogenRoutes(currentRoutes) {
  ...
  const childVirtualRoutes = virtualRoutes.map(({ path, file, index: index2, id }) => {
    return {
      file,
      id,
      index: index2,
      ...
      case "jsonLd": {
        ...
        for (const block of jsonLdBlocks) {
          ...
          const tag = generateTag(
            ...
            // @ts-expect-error
            `json-ld-${(block == null ? void 0 : block["@type"]) || (block == null ? void 0 : block.name) || index2++}`
          );
          tagResults.push(tag);
        }
        break;
...
var SeoLogger = (0, import_react28.lazy)(() => import("./log-seo-tags-IG37ONQ2-YBZDD4UH.js"));
function Seo({ debug }) {
